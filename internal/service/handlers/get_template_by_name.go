...
func GetTemplateByName(w http.ResponseWriter, r *http.Request) {
	...
	client, err := ClientQ(r).GetByName(request.User)
	if err != nil {
	...
	tmp, err := TemplateQ(r).GetByName(request.TemplateName, client.ID)
	if err != nil {
	...
	if tmp != nil {
		Log(r).Error(errors.Wrap(err, "template is not found"))
